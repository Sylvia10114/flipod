# Podcast Agent — 分层片段选择 Prompt 集

**版本**：v2.0 (2026-04-14)
**替换**：podcast_agent.py Step 4 原来的通用 prompt
**设计思路**：按播客来源的 tier（Business/Tech/Science/Story/Psychology/Culture）分派不同的评判标准，而不是一把梭
...
```python
def select_segments(transcript, podcast_name, tier, duration_minutes):
    """
    transcript: Whisper 词级输出
    podcast_name: 播客名（用于上下文）
    tier: 'Business'|'Tech'|'Science'|'Story'|'Psychology'|'Culture'
          从 CURATED_FEEDS 元数据取
    duration_minutes: 本集总时长
    """
    prompt = SHARED_PREAMBLE + TIER_PROMPTS[tier] + SHARED_OUTPUT_FORMAT
    # ... 发给 GPT，解析 JSON 输出
```

对于 tag 不明确的源（如 TED Radio Hour），先调用一次 **episode 分类 prompt**（见第八节）确定本集的 tier，再进入对应的片段选择 prompt。

---

## 一、共用前置规则（SHARED_PREAMBLE）

所有 tier 的 prompt 开头统一加上这一段：

```
你是一个英语听力内容的片段筛选器。任务：从一集播客的完整转录文本中，识别出 5-6 个候选片段（后续会由评估环节精选出最终的 3 个）。
...
1. **⚠️ 时长是硬约束——违反直接丢弃该候选，不要返回**
   ...
   **重要：不要返回"短但精彩"的 hook**——一个 35 秒的漂亮引语对产品无用，因为下游 filter 会直接拒绝。
   ...
   （v2.1 变更依据：dry-run 2026-04-14 有 93/123 候选 <45s 被 filter 直接拒绝，LLM 忽视了下限——原文"短而锐利也算合格"被误读为可以无视下限。）
...
3. **⚠️ 结尾完整性是硬约束——末尾不是完整句直接丢弃，不要返回**

   - 最后一个字符必须是 `.` `!` `?` 或引号（`"` `'`），**逗号结尾必须丢弃**
   - 禁止结尾：以 `and / but / or / because / which / that / to / of / in / on` 这类词结尾
   - 禁止结尾：半截短语悬空（"an amazing ...", "I can build ...", "For some of us, they ..." 这种听下去还没说完的）
   - 禁止结尾：广告/节目套话（"coming up after the break" / "sponsored by" / "subscribe" / "stay tuned" / "we'll be right back" / "our next story"）

   判断方法：把候选最后一句单独读出来，如果感觉"听众会下意识等下一句"说明没结束，必须扩展 end_time 到真正的句号处——或者整个候选作废。

   如果为了让结尾落到完整句要把 end_time 延长超过 duration 上限（见第 1 条），则**整个候选作废**，宁可少返回候选也不要返回结尾残缺的。

   （v2.2 变更依据：dry-run v2.1 有 9/46 个拒绝是 end_no_punctuation，目测其中 7-8 条是真半截。）
...
## 二、Business 类 Prompt (TIER_PROMPTS['Business'])

```
这集播客的 tier 是 Business/Finance。
...
## 三、Tech 类 Prompt (TIER_PROMPTS['Tech'])

```
这集播客的 tier 是 Technology。
...
Tech tier 的主要污染源是**播客主持人之间的承接闲聊**。这类开头的特征不是"用了 but/so/well 起头"（那是说话习惯，未必有问题），而是**句子里有明确指代前文的锚点**：
- "it's interesting what you just said..."  ← `what you just said` 明确指代前文
- "to your point about X..."  ← `to your point` 承接对方观点
- "that's a great example of..."  ← `that's` 指代前文提到的事
- "right, and so that's why..."  ← `right` + `that's` 双重承接

相比之下，这些开头虽然以连接词起步，但本身能作为独立 hook，应当保留：
- "But here's the thing most people miss about AI search..."  ← `But` 是戏剧性转折
- "So imagine you're Google's CEO in 2026..."  ← `So` 是叙事起手
- "Well, between Apple and Microsoft, there's a very famous moment late in Steve Jobs's career..."  ← 后面接的是故事开头

判断方法：把开头句单独拎出来读，如果会让人问"谁说了什么？"，淘汰；如果读完觉得"这是个有意思的开头"，保留。
...
## 四、Science 类 Prompt (TIER_PROMPTS['Science'])

```
这集播客的 tier 是 Science。
...
## 五、Story 类 Prompt (TIER_PROMPTS['Story'])

```
这集播客的 tier 是 Storytelling（The Moth、StoryCorps、Snap Judgment 等叙事型播客）。
...
## 六、Psychology 类 Prompt (TIER_PROMPTS['Psychology'])

```
这集播客的 tier 是 Psychology/Behavioral Science（Hidden Brain 等）。
...
## 七、Culture 类 Prompt (TIER_PROMPTS['Culture'])

```
这集播客的 tier 是 Culture/Society/History（Throughline、TED Radio Hour、StuffTheyDontWant 等）。
...
## 八、Episode 分类 Prompt（给通用源用）

对于 TED Radio Hour、NPR Up First、Freakonomics 这种话题跨度大的源，先跑一次 episode 分类，确定本集该用哪个 tier 的 prompt。

```
你是播客分类器。下面是一集播客的标题、描述和前 500 词转录。
...
## 九、共用输出格式（SHARED_OUTPUT_FORMAT）

所有 tier 的 prompt 末尾统一加上：
...
每次调整 prompt 请在文件顶部更新版本号和日期，并在 podcast_agent.py 里记录本次跑用的 prompt version，便于后续回溯哪批 clip 是哪版 prompt 生产的。

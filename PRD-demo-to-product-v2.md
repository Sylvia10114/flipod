# PRD v2: Listen Demo → 成品体验（参考 ListenLeap）
...
### 数据来源
localStorage `listenLeapBookmarks`（JSON 数组，每条：`{clipId, hookTitle, source, domain, tags[], timestamp}`）
...
### 数据来源
localStorage `listenLeapVocab`（JSON 数组，每条：`{word, cefr, phonetic, context, contextZh, timestamp}`）
...
### 切换逻辑
- 存 localStorage `listenLeapTheme`（值 `"dark"` 或 `"light"`）
- 切换入口在侧滑面板"设置"中
- 页面加载时读取并应用
...
- 存 localStorage `listenLeapLevel`

### Screen 2 — 兴趣标签选择
- 圆点导航：2 个点，第 2 个亮
- 底部小字：`随便选几个就行，AI 会从你的行为中学习`
- 原有的标签选择逻辑不变
- 存 localStorage `listenLeapInterests`
...
- `listenLeapName`、`listenLeapGender`、`listenLeapAge` 相关逻辑
- 圆点导航从 3 个改回 2 个
- 过渡屏中 "Hi {昵称}" 相关文案
...
### 数据存储
- 收藏词 → localStorage `listenLeapVocab`
- 认识词 → localStorage `listenLeapKnownWords`
- API 结果缓存到内存 Map
...
**数据来源**：`listenLeapVocab` 中收藏时间 ≥ 3 天的词
...
**间隔重复数据**：存 localStorage `listenLeapReview`，每条：`{word, nextReview, interval}`
...
- "好的" → `listenLeapPreferences` 中该 tag 权重 +1
- "保持现状" → 不操作
...
### 爱心（❤️）= 喜欢这类内容
- 点击后存入 localStorage `listenLeapLikes`：`{clipId, tags[], timestamp}`
- 用于触发推荐调整卡（见 8.3）
- 页面刷新后从 localStorage 恢复点亮状态

### 书签（🔖）= 收藏片段
- 点击后存入 localStorage `listenLeapBookmarks`：`{clipId, hookTitle, source, domain, tags[], timestamp}`
- 可在侧滑面板"我的收藏"子页面查看
- 取消收藏时从数组中移除
- 页面刷新后恢复状态
...
- data.json 结构不变
- 所有用户数据存 localStorage，key 统一 `listenLeap` 前缀
...
| Key | 类型 | 用途 |
|-----|------|------|
| `listenLeapLevel` | string | CEFR 等级 |
| `listenLeapInterests` | JSON array | 兴趣标签 |
| `listenLeapSpeed` | string | 播放速度 |
| `listenLeapTheme` | string | 主题（dark/light） |
| `listenLeapLikes` | JSON array | 爱心记录 {clipId, tags[], timestamp} |
| `listenLeapBookmarks` | JSON array | 收藏记录 {clipId, hookTitle, source, domain, tags[], timestamp} |
| `listenLeapVocab` | JSON array | 收藏词汇 {word, cefr, phonetic, context, contextZh, timestamp} |
| `listenLeapKnownWords` | JSON array | 标记为"认识"的词 |
| `listenLeapReview` | JSON array | 间隔重复数据 {word, nextReview, interval} |
| `listenLeapPreferences` | JSON object | 推荐偏好权重 {tag: weight} |
